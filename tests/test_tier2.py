...
def test_sfm_gets_real_frames_not_thumbnails():
    ...
    """
    from frameflow import filmindex as fx
...
def test_single_setup_deadlock():
    ...
    check("run() is the caller that opens the door",
          src in (_ROOT / 'frameflow' / 'render.py').read_text(encoding='utf-8'))
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check("and process_shot reads that name",
              "model.sparse_dir" in (_ROOT / 'frameflow' / 'render.py').read_text(encoding='utf-8'))
...
def test_external_reference_rung():
    ...
    from frameflow import fetchers as ft
