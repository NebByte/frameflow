...
def _numbers_match_pixels(summary, provs, deliv):
    ...
    from frameflow import polish as polish_mod
    shares = []
    for p in provs:
        try:
        ...
        for pm in stack[::8]:
            ...
            ww, _fps = polish_mod.dims(summary, w)
            if ww < 2:
                continue
...
def _polish(job_dir, summary, segs):
    ...
    from frameflow import polish  # ---- the free pass first, because that is what the button now does
